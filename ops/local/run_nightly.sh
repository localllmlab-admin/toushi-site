...
# 2.5 最終レビュー・監査・ファクトチェック（必須工程・Masaru指示2026-07-08。不合格は公開経路に乗せない）
node ops/local/final_audit.mjs "$OUT" || {
  echo "最終監査FAIL: 生成物を退避 $(date -Is)"
  mkdir -p ops/local/rejected; mv "$OUT" ops/local/rejected/
  git checkout -- ops/hermes/topics_backlog.md 2>/dev/null || true  # トピックを未消化に戻す
  exit 0
}
