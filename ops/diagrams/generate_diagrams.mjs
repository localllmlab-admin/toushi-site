...
/* ============ 161. ギャンアングル ============ */
{
  let g = txt(320, 24, "ギャンアングル：起点から「時間×価格」の角度で線を引く", { anchor: "middle", bold: true, fill: INK, size: 12.5 });
  const o = [80, 260];
  g += line(o[0], o[1], 300, 60, { color: UP, w: 2 });
  g += txt(295, 52, "2×1（急）", { size: 10, fill: UP });
  g += line(o[0], o[1], 480, 70, { color: LV, w: 2.5 });
  g += txt(490, 68, "1×1（45度）", { size: 10.5, fill: LV, bold: true });
  g += line(o[0], o[1], 580, 160, { color: DN, w: 2 });
  g += txt(555, 178, "1×2（緩）", { size: 10, fill: DN });
  g += poly([[80, 260], [140, 215], [190, 235], [260, 170], [320, 195], [400, 130], [460, 150], [540, 100]], { color: INK, w: 2.2 });
  g += txt(320, 292, "1×1線（時間1に対して価格1）の上なら強い相場、割れたら次の角度へ――という段階的な読み方をする（模式図）", { anchor: "middle", size: 10.5, fill: SUB });
  save("gann-angles", 640, 305, g);
}

/* ============ 162. ファン理論・スピードライン ============ */
{
  let g = txt(320, 24, "ファン理論：引き直した3本目のラインを割れたら転換の目安", { anchor: "middle", bold: true, fill: INK, size: 12.5 });
  const o = [70, 265];
  g += line(o[0], o[1], 560, 55, { color: UP, w: 1.8, dash: "2 3" });
  g += txt(566, 58, "①", { size: 11, fill: UP, bold: true });
  g += line(o[0], o[1], 560, 125, { color: LV, w: 1.8, dash: "2 3" });
  g += txt(566, 128, "②", { size: 11, fill: LV, bold: true });
  g += line(o[0], o[1], 560, 195, { color: DN, w: 1.8, dash: "2 3" });
  g += txt(566, 198, "③", { size: 11, fill: DN, bold: true });
  g += poly([[70, 265], [150, 175], [210, 120], [260, 90], [300, 130], [340, 110], [380, 160], [420, 145], [470, 200], [520, 230]], { color: INK, w: 2.2 });
  g += arrow(470, 240, 500, 218, { color: DN, w: 2 });
  g += txt(470, 262, "③割れ＝上昇の構造が崩れた", { anchor: "middle", size: 10.5, fill: DN });
  g += txt(320, 292, "同じ起点からライン割れのたびに引き直す。3本目の割れを転換の警告とみなす（スピードラインも同発想・模式図）", { anchor: "middle", size: 10.5, fill: SUB });
  save("fan-speed-lines", 640, 305, g);
}

/* ============ 163. 移動平均の種類 ============ */
{
  let g = txt(320, 24, "SMAとEMA：同じ期間でも「反応の速さ」が違う", { anchor: "middle", bold: true, fill: INK, size: 12.5 });
  g += poly([[60, 220], [120, 218], [180, 215], [240, 210], [280, 140], [340, 110], [400, 95], [460, 85], [540, 75]], { color: INK, w: 2.2 });
  g += txt(65, 205, "価格（急伸）", { size: 10.5, fill: INK });
  g += poly([[60, 235], [120, 234], [180, 232], [240, 228], [280, 195], [340, 160], [400, 135], [460, 118], [540, 103]], { color: UP, w: 2.2 });
  g += txt(548, 106, "EMA", { size: 10.5, fill: UP, bold: true });
  g += poly([[60, 240], [120, 239], [180, 238], [240, 236], [280, 220], [340, 195], [400, 172], [460, 150], [540, 130]], { color: DN, w: 2.2 });
  g += txt(548, 134, "SMA", { size: 10.5, fill: DN, bold: true });
  g += arrow(310, 250, 310, 180, { color: SUB, w: 2 });
  g += txt(318, 245, "EMAは直近の価格を重く見るため追従が速い（その分だましも拾いやすい）", { size: 10 });
  g += txt(320, 290, "SMA＝全期間を均等に平均／EMA＝直近を指数的に重視／WMA＝直近ほど大きな比重。速さと安定のトレードオフ（模式図）", { anchor: "middle", size: 10, fill: SUB });
  save("ma-types", 640, 305, g);
}

/* ============ 164. CCI ============ */
{
  let g = txt(320, 24, "CCI：平均的な値動きからの乖離を測る（±100が基準帯）", { anchor: "middle", bold: true, fill: INK, size: 12.5 });
  g += poly([[60, 100], [130, 85], [200, 95], [270, 70], [340, 80], [420, 55], [500, 65], [560, 45]], { color: INK, w: 2.2 });
  g += txt(65, 60, "価格", { size: 10.5, fill: INK });
  g += line(40, 125, 600, 125, { color: GRID, w: 1.5 });
  g += line(60, 165, 580, 165, { color: UP, w: 1.5, dash: "6 4" });
  g += txt(588, 169, "+100", { size: 10, fill: UP, bold: true });
  g += line(60, 205, 580, 205, { color: INK, w: 1, dash: "3 4" });
  g += txt(588, 209, "0", { size: 10 });
  g += line(60, 245, 580, 245, { color: DN, w: 1.5, dash: "6 4" });
  g += txt(588, 249, "−100", { size: 10, fill: DN, bold: true });
  g += poly([[60, 215], [130, 185], [200, 200], [270, 150], [340, 175], [420, 140], [500, 190], [560, 225]], { color: LV, w: 2.2 });
  g += txt(320, 285, "+100超は「平均的な変動幅を上回る強さ」。トレンド発生のサインとも過熱とも読まれ、使い方が分かれる（模式図）", { anchor: "middle", size: 10.5, fill: SUB });
  save("cci", 640, 300, g);
}

/* ============ 165. ウィリアムズ%R ============ */
{
  let g = txt(320, 24, "ウィリアムズ%R：期間中の高値・安値レンジのどこにいるか", { anchor: "middle", bold: true, fill: INK, size: 12.5 });
  g += poly([[60, 100], [130, 80], [200, 92], [270, 65], [340, 85], [420, 60], [500, 75], [560, 55]], { color: INK, w: 2.2 });
  g += txt(65, 60, "価格", { size: 10.5, fill: INK });
  g += line(40, 125, 600, 125, { color: GRID, w: 1.5 });
  g += line(60, 155, 580, 155, { color: UP, w: 1.5, dash: "6 4" });
  g += txt(588, 159, "−20", { size: 10, fill: UP, bold: true });
  g += line(60, 245, 580, 245, { color: DN, w: 1.5, dash: "6 4" });
  g += txt(588, 249, "−80", { size: 10, fill: DN, bold: true });
  g += poly([[60, 230], [130, 165], [200, 185], [270, 140], [340, 195], [420, 150], [500, 235], [560, 255]], { color: LV, w: 2.2 });
  g += txt(140, 145, "−20より上＝買われすぎ圏", { size: 10, fill: UP });
  g += txt(420, 268, "−80より下＝売られすぎ圏", { size: 10, fill: DN });
  g += txt(320, 292, "0〜−100で表示（0が期間中の高値張り付き）。ストキャスティクスの%Kと上下が反転した関係（模式図）", { anchor: "middle", size: 10.5, fill: SUB });
  save("williams-r", 640, 305, g);
}

/* ============ 166. サイコロジカルライン ============ */
{
  let g = txt(320, 24, "サイコロジカルライン：直近12日のうち上昇した日の割合", { anchor: "middle", bold: true, fill: INK, size: 12.5 });
  for (let i = 0; i < 12; i++) {
    const up = [0, 1, 3, 4, 5, 7, 8, 9, 10, 11].includes(i);
    g += `<rect x="${70 + i * 42}" y="60" width="34" height="26" rx="4" fill="${up ? UP : DN}" opacity="0.75"/>`;
    g += txt(87 + i * 42, 78, up ? "陽" : "陰", { anchor: "middle", size: 11, fill: "#ffffff", bold: true });
  }
  g += txt(320, 112, "上の例：12日中10日上昇 → 10÷12 ≒ 83%", { anchor: "middle", size: 11, fill: INK, bold: true });
  g += line(60, 155, 580, 155, { color: UP, w: 1.5, dash: "6 4" });
  g += txt(588, 159, "75%", { size: 10, fill: UP, bold: true });
  g += line(60, 215, 580, 215, { color: DN, w: 1.5, dash: "6 4" });
  g += txt(588, 219, "25%", { size: 10, fill: DN, bold: true });
  g += poly([[70, 200], [140, 175], [210, 148], [280, 140], [350, 168], [420, 195], [490, 222], [560, 205]], { color: LV, w: 2.2 });
  g += txt(320, 258, "値幅を見ず「勝ち負けの回数」だけを見る単純な指標。75%超・25%割れが行き過ぎの目安とされる（模式図）", { anchor: "middle", size: 10.5, fill: SUB });
  save("psychological-line", 640, 272, g);
}

/* ============ 167. モメンタム・ROC ============ */
{
  let g = txt(320, 24, "モメンタム：価格より先に「勢い」が頭打ちになることがある", { anchor: "middle", bold: true, fill: INK, size: 12.5 });
  g += poly([[60, 230], [130, 190], [200, 150], [270, 115], [340, 90], [420, 72], [500, 62], [560, 58]], { color: INK, w: 2.2 });
  g += txt(65, 175, "価格：まだ上昇中", { size: 10.5, fill: INK });
  g += line(40, 130, 600, 130, { color: GRID, w: 1.5 });
  g += poly([[60, 250], [130, 225], [200, 195], [270, 175], [340, 168], [420, 175], [500, 195], [560, 215]], { color: LV, w: 2.2 });
  g += `<circle cx="340" cy="168" r="6" fill="none" stroke="${DN}" stroke-width="2"/>`;
  g += txt(348, 158, "モメンタムはピークアウト＝上昇の速度が鈍り始めた", { size: 10, fill: DN });
  g += txt(320, 288, "モメンタム＝当日終値−n日前終値（ROCは比率表示）。加速度計として「勢いの変化」を先に検出する発想（模式図）", { anchor: "middle", size: 10.5, fill: SUB });
  save("momentum-roc", 640, 302, g);
}

/* ============ 168. アイランドリバーサル ============ */
{
  let g = txt(320, 24, "アイランドリバーサル：窓で孤立した「島」ができる反転形", { anchor: "middle", bold: true, fill: INK, size: 12.5 });
  g += candle(90, 200, 250, 210, 255, { w: 15 });
  g += candle(125, 175, 225, 185, 230, { w: 15 });
  g += candle(160, 150, 200, 160, 205, { w: 15 });
  g += candle(195, 130, 178, 140, 182, { w: 15 });
  // 窓を開けて上放れ（島の部分）
  g += candle(265, 75, 105, 85, 110, { w: 15 });
  g += candle(300, 65, 92, 100, 105, { w: 15 });
  g += candle(335, 70, 88, 98, 103, { w: 15 });
  // 窓を開けて下放れ
  g += candle(405, 135, 165, 185, 190, { w: 15 });
  g += candle(440, 165, 195, 215, 220, { w: 15 });
  g += candle(475, 190, 220, 245, 250, { w: 15 });
  g += `<rect x="240" y="55" width="120" height="65" rx="8" fill="none" stroke="${LV}" stroke-width="2" stroke-dasharray="6 4"/>`;
  g += txt(300, 48, "島（アイランド）", { anchor: "middle", size: 11, fill: LV, bold: true });
  g += txt(228, 132, "窓↑", { anchor: "middle", size: 11, fill: UP, bold: true });
  g += txt(372, 132, "窓↓", { anchor: "middle", size: 11, fill: DN, bold: true });
  g += txt(320, 292, "上放れの窓と下放れの窓に挟まれ、高値圏の数本が孤立する。買い方の総退却を映す反転サインとされる（模式図）", { anchor: "middle", size: 10.5, fill: SUB });
  save("island-reversal", 640, 305, g);
}

/* ============ 169. サイクル分析 ============ */
{
  let g = txt(320, 24, "サイクル分析：底から底までの周期と「山の位置」を観察する", { anchor: "middle", bold: true, fill: INK, size: 12.5 });
  const wave = [];
  for (let i = 0; i <= 100; i++) {
    const x = 60 + i * 5;
    const y = 160 - Math.sin(i / 8) * 55 - i * 0.25;
    wave.push([x, y]);
  }
  g += poly(wave, { color: INK, w: 2.2 });
  g += line(85, 250, 287, 250, { color: LV, w: 2 });
  g += arrow(287, 250, 287, 232, { color: LV, w: 1.5 });
  g += arrow(85, 250, 85, 232, { color: LV, w: 1.5 });
  g += txt(186, 268, "1周期（底から底まで）", { anchor: "middle", size: 10.5, fill: LV, bold: true });
  g += txt(320, 292, "上昇トレンド中は山が周期の後半に寄る（右方移転）とされる。周期は揺らぐため「およその目安」で使う（模式図）", { anchor: "middle", size: 10.5, fill: SUB });
  save("cycle-analysis", 640, 305, g);
}

/* ============ 170. 相対株価 ============ */
{
  let g = txt(320, 24, "相対株価＝銘柄（業種）÷市場全体：市場に勝っているかを測る", { anchor: "middle", bold: true, fill: INK, size: 12.5 });
  g += poly([[60, 110], [140, 100], [220, 108], [300, 95], [380, 102], [460, 92], [560, 98]], { color: INK, w: 2.2 });
  g += txt(70, 82, "銘柄の株価（横ばいに見える）", { size: 10.5, fill: INK });
  g += line(40, 140, 600, 140, { color: GRID, w: 1.5 });
  g += poly([[60, 250], [140, 235], [220, 225], [300, 205], [380, 190], [460, 172], [560, 155]], { color: UP, w: 2.5 });
  g += txt(70, 215, "相対株価（÷TOPIX）は右肩上がり＝市場平均より強い", { size: 10.5, fill: UP, bold: true });
  g += txt(320, 288, "全体が下げる中で下げ渋る銘柄は、相対株価では上昇して見える。「強い業種・銘柄」の選別に使う（模式図）", { anchor: "middle", size: 10.5, fill: SUB });
  save("relative-strength", 640, 302, g);
}

/* ============ 171. サプライズと行き過ぎ ============ */
{
  let g = txt(320, 24, "サプライズへの過剰反応：行き過ぎてから修正される", { anchor: "middle", bold: true, fill: INK, size: 12.5 });
  g += line(60, 190, 580, 190, { color: GRID, w: 1.5 });
  g += line(340, 55, 340, 260, { color: LV, w: 2, dash: "6 4" });
  g += txt(340, 47, "サプライズ発表", { anchor: "middle", size: 11, fill: LV, bold: true });
  g += line(400, 120, 580, 120, { color: SUB, w: 1.5, dash: "3 4" });
  g += txt(500, 110, "新しい妥当水準（のイメージ）", { anchor: "middle", size: 9.5 });
  g += poly([[60, 195], [130, 190], [200, 193], [270, 188], [335, 190]], { color: INK, w: 2.2 });
  g += poly([[335, 190], [345, 130], [355, 85], [370, 70]], { color: UP, w: 2.5 });
  g += poly([[370, 70], [400, 95], [430, 115], [470, 128], [520, 122], [570, 118]], { color: DN, w: 2.2 });
  g += txt(388, 60, "初動で行き過ぎる", { size: 10.5, fill: UP });
  g += txt(480, 148, "妥当水準へ向けて修正", { size: 10.5, fill: DN });
  g += txt(320, 290, "急騰の全部が実力ではない。初動の飛び付きは「行き過ぎの分」を高値づかみするリスクを伴う（模式図）", { anchor: "middle", size: 10.5, fill: SUB });
